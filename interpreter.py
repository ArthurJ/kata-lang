...
class Scope:
    def __init__(
        self, values=None, functions=std_lib_funs, cache=None, outer_scope=None
    ) -> None:
...
def is_literal(val):
    if isinstance(val, tuple):
        return val[0] in ["STR", "INT", "FLOAT", "BOOL"]
    return False
...
def interpret(token_list, scope=scope):
    ...
    if root[0] == "LET":
        ...
        return [root, interpret(token_list, scope)]
...
def bind(symb, val, scope=scope):  # sempre guarda expressões
    if not symb[0] == "SYMB":
        raise ValueError("To bind a symbol is needed")
...
def eval(stack, scope=scope):
    ...
    if not stack:
        return ""
    if is_literal(stack):
        return literal_val(stack)
    if stack[0] == "LET":
        bind(*stack[1], scope)
        return stack
    if callable(stack[0]):
        scope.cache[stack] = scope.cache.get(
            stack, stack[0](*[eval(x, scope) for x in stack[1]])
        )
